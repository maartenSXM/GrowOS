...
void calcDaylightNight(){
  ...
  if(!(devIsState[0][1]) && !(devIsAuto[0][1])){                         //sun not on in slot 0, maybe dry mode...
    daylight = 0;
  }
  else if ((devIsState[0][1]) || (devIsAuto[0][1])){                                                                  //sun is on in slot 0
    if(startTimeSlots[1].hour > startTimeSlots[0].hour) daylight = startTimeSlots[1].hour - startTimeSlots[0].hour;   //slot 1 ends in same day
    else daylight = (24 - startTimeSlots[0].hour) + startTimeSlots[1].hour;                                           //slot 1 ends in next day     
  }
  
  night = 24-daylight;
}

void saveSlotTimes(){                             //requires sunriseTime.hour and daylight to be set
...
void debugSchedule(){
  ...
  Serial.print("Light on for seconds: ");Serial.println(countDeviceOnTime(1));
...
void checkSlot(){
  ...
  if(withinCurrentSlot()){       //enters if in currentSlot Still - check if a device needs to be periodically altered if auto, or if set to be periodic
    updateDeviceStates();
  }
  else{                   //move to next slot...or the slot after that if we missed one...
    findCurrentSlot();
    startNewSlot();
  }
}
...
void setDeviceState() {
  if (!devIsAuto[currentSlot][0] && !devIsPeriodic[currentSlot][0] && devIsState[currentSlot][0]) setFanPWM(fanSpeed);
  if (!devIsAuto[currentSlot][1] && !devIsPeriodic[currentSlot][1]) setPin(ACT_LED, devIsState[currentSlot][1]);
  if (!devIsAuto[currentSlot][2] && !devIsPeriodic[currentSlot][2]) setPin(ACT_PUMP, devIsState[currentSlot][2]);
  if (!devIsAuto[currentSlot][3] && !devIsPeriodic[currentSlot][3]) setPin(ACT_5V, devIsState[currentSlot][3]);
}

//for auto and periodically on devices, manually set devices should already be set
void updateDeviceStates(){           
  //Fan
  if(devIsAuto[currentSlot][0]) setDeviceAuto(0);
  else if(devIsPeriodic[currentSlot][0]) setDevicePeriodic(0);   
   
  //Light
  if(devIsAuto[currentSlot][1]) setDeviceAuto(1); 
  else if(devIsPeriodic[currentSlot][1]) setDevicePeriodic(1);
  
  //Pump
  if(devIsAuto[currentSlot][2]) setDeviceAuto(2);
  else if(devIsPeriodic[currentSlot][2]) setDevicePeriodic(2); 
  
  //12V
  if(devIsAuto[currentSlot][3]) setDeviceAuto(3);
  else if(devIsPeriodic[currentSlot][3]) setDevicePeriodic(3); 
  
  //5V
  if(devIsAuto[currentSlot][4]) setDeviceAuto(4);
  else if(devIsPeriodic[currentSlot][4]) setDevicePeriodic(4); 
}
...
bool withinCurrentSlot(int8_t slot) {
...
#ifdef DEBUG
    Serial.println("***********ERROR************ \n NOT A VALID SLOT NUMBER: ");Serial.println(slot);
#endif
    slot = slot % numSlots;
  }
  if(numSlots <= 1){
  ...
  if((numSlots == 2) && isSameTime(SS, SE)){                  //means either is all daylight (slot 0), or all night (slot 1)
    if((daylight == 24) && (slot == 0)) result = true;
    else if((night == 24) && (slot == 1)) result = true;
    else result = false;
  }
  else if (
...
#ifdef DEBUG
   if(result)Serial.println("YES in slot: " + String(slot));
   else Serial.println("NOT in slot: " + String(slot));
#endif 
  return result;
}
...
void insertTimeSlot(_Time tS){
  if(numSlots == maxNumSlots) return;
  if(timeSlotExists(tS)) return;          //if same hour and min as another exit

  int slot = 0;
  for(int i = 0; i <numSlots; i++){        //find the slot index val it will go into
  ...
  for(int i = numSlots; i > slot; i--){
    for(int j = 0; j < numDevices; j++) devIsAuto[i][j] = devIsAuto[i-1][j];
    for(int j = 0; j < numDevices; j++) devIsState[i][j] = devIsState[i-1][j];
    for(int j = 0; j < numDevices; j++) devIsPeriodic[i][j] = devIsPeriodic[i-1][j];
    for(int j = 0; j < numDevices; j++){for(int k = 0; k < 4; k++) devPeriodic[i][j][k] = devPeriodic[i-1][j][k];};
  ...
  int fromSlot;
  if(slot == 0) fromSlot = slot + 1;      //first slot
  else fromSlot = slot-1;                 //middle or last slot
  for(int j = 0; j < numDevices; j++) devIsAuto[slot][j] = devIsAuto[fromSlot][j];
  for(int j = 0; j < numDevices; j++) devIsState[slot][j] = devIsState[fromSlot][j];
  for(int j = 0; j < numDevices; j++) devIsPeriodic[slot][j] = devIsPeriodic[fromSlot][j];
  for(int j = 0; j < numDevices; j++){for(int k = 0; k < 4; k++) devPeriodic[slot][j][k] = devPeriodic[fromSlot][j][k];};
  startTimeSlots[slot] = tS;   
}


void removeTimeSlot(_Time tS){
  if(numSlots == 1) return;                //cannot have less than 1 slot
  if(!timeSlotExists(tS)) return;          //timeslot does not exist

  int slot = 0;
  for(int i = 0; i <numSlots; i++){        //find the slot index val to be removed
...
bool isBefore(_Time t1, _Time t2){
  if((t1.hour < t2.hour) ||
    ...
    ((t1.hour == t2.hour) && (t1.minute == t2.minute) && (t1.second < t2.second))) return true;
  else  return false;
}
  ...
  if((numSlots == 2) && (devIsState[0][devI] || devIsAuto[0][devI]) && (devIsState[1][devI] || devIsAuto[1][devI])){
    ...
    for (int slot = 0; slot < numSlots; slot++) {
      if(devIsState[slot][devI] && (!devIsPeriodic[slot][devI])) {
        if ( isBefore(startTimeSlots[slot], startTimeSlots[(slot+1)%numSlots])) seconds += secondsDiffTwoTimes(startTimeSlots[slot], startTimeSlots[(slot+1)%numSlots]);
        //else a the end time is seemingly before the start because it wrapped through a new day;
        else seconds += secondsDiffTwoTimesNewDay(startTimeSlots[slot], startTimeSlots[(slot+1)%numSlots]);
      }
...
int secondsSinceSunrise(_Time cT) {                                 //if sunrise not yet, returns the time to sunrise as negative
  int sunriseTimeSeconds = 0;
  int elapsedTime = 0;
  for (int slot = 0; slot < numSlots; slot++) {                     //find the first slot the light([1]) is on, that is when sunRise is
...
int secondsSinceSunset(_Time cT){
  ...
  int elapsedTime = 0;
  for(int slot = 0; slot < numSlots; slot++){                     //find the first slot the light([1]) is Off, that is when sunset is
    if((!devIsAuto[slot][1]) && (!devIsState[slot][1])) {
...
int secondsDiffTime(_Time sT){    //default to current time
  pollCurrentTime();
  int seconds = 0;
  int hours = 0 ;
  int minutes = 0;
...
struct _Time timeAdjustSeconds(_Time timeAdd, int seconds){
  if(seconds == 0) return timeAdd;
  if(seconds > 0){                                      //positive seconds
    if(seconds < (60 - timeAdd.second)){                //the seconds to be added fit in this minute
      timeAdd.second = timeAdd.second + seconds;
    }
    else{                                               //the seconds dont fit
  ...
  else {                                                //seconds are negative 
    seconds = seconds * -1;
    if(seconds <= timeAdd.second){                      //the seconds to be removed fit in this minute
      timeAdd.second = timeAdd.second - seconds;
    }
    else{                                               //the seconds dont fit
      int multiple = seconds/60;
...
_Time timeAdjustMinutes(_Time timeAdd,int minutes){
  if(minutes == 0) return timeAdd;
  if(minutes > 0){                                      //positive minutes
    if(minutes < (60 - timeAdd.minute)){                //the minutes to be added fit in this minute
      timeAdd.minute = timeAdd.minute + minutes;
    }
    else{                                               //the minutes dont fit
  ...
  else {                                                //minutes are negative 
    minutes = minutes * -1;
    if(minutes <= timeAdd.minute){                      //the minutes to be removed fit in this minute
      timeAdd.minute = timeAdd.minute - minutes;
    }
    else{                                               //the minutes dont fit
      int multiple = minutes/60;
