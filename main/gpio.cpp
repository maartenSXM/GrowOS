...
void setPin(int pin, int level){
  ...
  Serial.print(F(" - "));if(level)Serial.println("HIGH");else Serial.println("LOW");
#endif
  if(level){
    pinMode(pin, OUTPUT);
...
void allDevicesOff(){
  for(int i = 1; i<=numDevices; i++){             //start 1 becuase fan(0) has no pin
...
void setDeviceAuto(const int devI){
  ...
  for(int i = 0; i < numSlots; i++){                                          //remove auto setting and set so quota is filled
