...
void drawScheduleDevices(int slot, int x, int y, int slotWidth)
{
  ...
  { // if auto draw...
    ...
    if (devIsAuto[slot][i])
    {
      if (slotWidth > (radius * 2 + 2))
        tft.drawCircle(x + cx, y + cy, radius, YELLOW);
      if (slotWidth > textAndCircle)
        tft.drawString(strTemp2, x + tx, y + ty);
    }
    else if (devIsPeriodic[slot][i])
    { // if periodic draw...
      if (slotWidth > (radius * 2 + 2))
        tft.drawCircle(x + cx, y + cy, radius, GREEN);
      if (slotWidth > textAndCircle)
        tft.drawString(strTemp2, x + tx, y + ty);
    }
    else
    { // else draw based on the manual setting
      if (devIsState[slot][i])
      {
        if (slotWidth > (radius * 2 + 2))
          tft.fillCircle(x + cx, y + cy, radius, GREEN);
        if (slotWidth > textAndCircle)
          tft.drawString(strTemp2, x + tx, y + ty);
      }
      else
      {
        if (slotWidth > (radius * 2 + 2))
          tft.drawCircle(x + cx, y + cy, radius, DARKGREY);
        if (slotWidth > textAndCircle)
          tft.drawString(strTemp2, x + tx, y + ty);
      }
    }
...
void schedulePage()
{
  ...
  if (!updatePageOnly)
  {
    ...
    { // The timeSlot bars vertical line, time stamps, and schedule details
      ...
      tft.drawRect(rectLeft, yMinTL + 5, 38, 12, timeSlotColor);
      tft.setFreeFont(NULL);
      tft.setTextSize(0.5);
      sprintf(strTemp, "%02d:%02d", startTimeSlots[i].hour, startTimeSlots[i].minute);
      tft.drawString(strTemp, rectLeft + 5, yMinTL + 7);

      if (numSlots == 1)
      { // the first one and only
        if (x <= (xDisp / 2))
          drawScheduleDevices(i, x, yMaxTL, xDisp / 2); // if slot mostly on left, draw right of slot
        else
          drawScheduleDevices(i, xMinTL, yMaxTL, xDisp / 2); // if slot mostly on right, draw on far left
      }
      ...
      { // the last
        ...
        if (diffToEnd > startToFirst)
          drawScheduleDevices(i, x, yMaxTL, diffToEnd); // the space at the end of the schedule is larger than the start to first
        else
          drawScheduleDevices(i, xMinTL, yMaxTL, startToFirst);
      }
    }
    tft.setFreeFont(&FreeSans9pt7b);
  }
...
void daysSelectPage()
{
  ...
  { // Set button colours based on if ON or OFF or HIDDEN
  ...
  if (checkTPOnly)
  {
    ...
    for (i = 0; i < numDayBtns; i++)
    {
      if ((i == 0) && awaitingButtonRelease(&dayBtns[i]))
      { // Look at the All button first
        if (daysSelected[0] == 0)
        {
      ...
      else if (daysSelected[1] != 2)
      { // Now lets look at the lower res buttons that are not hidden
        ...
        if ((numDaysSelected == 1) && dayBtns[lastDay].contains(tp.x, tp.y))
        { // Dont disable the last remaining day button to be turned off
          return;
        }
      ...
      { // The buttons are hidden, If touch a button only select that one.
        if (awaitingButtonRelease(&dayBtns[i]))
        {
          for (j = 0; j < numDayBtns; j++)
          { // all buttons off except...
            daysSelected[j] = 0;
          }
  ...
  if (!updatePageOnly && !reDrawStuff)
  {
    ...
    exitBtn.drawButton();
    tft.drawBitmap(430, 2, timeline50x50, 50, 50, OPBOX_GREEN, 1); // exit page btn
    reDrawStuff = true;
  }
...
void weeksSelectPage()
{
  ...
  if (checkTPOnly)
  {
    ...
    for (i = 0; i < numWeekBtns; i++)
    {
      if ((i == 0) && awaitingButtonRelease(&weekBtns[i]))
      { // Look at the All button first
      ...
      else if (weeksSelected[1] != 2)
      { // Now lets look at the lower res buttons if they are not hidden
        ...
        if (numWeeksSelected < 1)
        { // should never happen
          weeksSelected[0] = 1;
          reDrawPage();
          return;
        }
        if ((numWeeksSelected == 1) && weekBtns[lastWeek].contains(tp.x, tp.y))
        { // Dont disable the last remaining week button to be turned off
          return;
        }
...
{
  // tft.setFreeFont(NULL);
  tft.setFreeFont(&FreeSans24pt7b); // The new timeslot time
  char buff[6];
  strcpy(buff, "");
...
void newTimeslotPage()
{
  ...
  {                        // only runs if we are building page from scratch, enters here if you call the page function directly without going through updatePage()
    ...
    exitBtn.drawButton();
    tft.drawBitmap(430, 2, check50x50, 50, 50, GREY);
