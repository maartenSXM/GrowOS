...
void homePage(){
    ...
    for(int slot = 0; slot<numSlots; slot++){
      if(devIsAuto[slot][1] == 1 || devIsState[slot][1] == 1){          //find the first slot the light is on, that is when sunRise is
    ...
    //settings button
    tft.drawBitmap8(465, 40, setting50x50, 50, 50, GREY, 0.5); 
    ...
    if (stemp.length() > 1 && stemp.length() < 3 && shumidity.length() > 1 && shumidity.length() < 3){
        ...
        tft.setFont(NULL);
        tft.print("o", middle-middle/2+tempLength, tempHumidityY-4);
        tft.setFont(&FreeSans9pt7b);
        tft.print(stempUnit, middle-middle/2+tempLength+5, tempHumidityY);
        tft.print("%", middle+middle/2, tempHumidityY);
        tft.setFont(NULL);
        ...
        tft.setFont(NULL);
        tft.print("o", middle-middle/2+tempLength+tempLength/2, tempHumidityY-4);
        tft.setFont(&FreeSans9pt7b);
        tft.print(stempUnit, middle-middle/2+tempLength+5+tempLength/2, tempHumidityY);
        tft.print("%", middle+middle/2, tempHumidityY);
        tft.setFont(NULL);
        ...
        tft.setFont(&FreeSans9pt7b);
        tft.print(stempUnit, middle-middle/2+tempLength+5, tempHumidityY);
        tft.print("%", middle+middle/2-humidityLength/2, tempHumidityY);
        tft.setFont(NULL);
        ...
        tft.setFont(NULL);
        tft.print("Humidity: Err 420", middle+middle/6, tempHumidityY);
        tft.print("Temp: Err 420", middle-middle/2, tempHumidityY);
    }
...
void splashPage(){
  ...
  tft.drawBitmap8(50, 50, tractor_128x64, 128, 64, BLUE);  // for 8bit monochrome bit map works. draws from top left of image
