...
void TFT_eSPI::drawBitmapScale(int16_t x, int16_t y, const uint8_t bitmap[],
                                    int16_t w, int16_t h, uint16_t color, float scale)
{
  int w2 = w * scale;
  int h2 = h * scale;
  uint8_t sBitmap[w2 * h2];    // the scaled bitmap
  uint8_t tempBitmap[w * h]; // the scaled bitmap
  ...
  for (int a = 0; a < h2; a++)
  {
    for (int b = 0; b < w2; b++)
    {
      px = floor(b * x_ratio);
      py = floor(a * y_ratio);
      sBitmap[(a * w2) + b] = tempBitmap[(int)((py * w) + px)]; // pgm_read_byte(&bitmap[(int)((py* w * byteWidth) + px / 8)]);
    }
  }
  startWrite();
...
void TFT_eSPI::setColor(uint16_t c)
{
  // For 'transparent' background, we'll set the bg
  // to the same as fg instead of using a flag
  textcolor = textbgcolor = c;
