...
void app_main(void)
{
    ...
    nvs_open("DeviceMetrics", NVS_READWRITE, &my_handle);

    // Read
    int32_t power_up_count = 0;
    nvs_get_i32(my_handle, POWER_UP_COUNT_KEY, &power_up_count);

    // Increment and Save
    power_up_count++;
    nvs_set_i32(my_handle, POWER_UP_COUNT_KEY, power_up_count);
    nvs_commit(my_handle);
    nvs_close(my_handle);

    // So to not block CPU 1 programming, Conditional Serial Port Deactivation and Pin Configuration
    if (power_up_count <= 2)
    {
        // Deactivate Serial Port
        Serial.end();

        // Set Serial / GPIO pins to floating state so not to interfere with CPU 1 programming
        gpio_reset_pin(H_U0TXD);
        gpio_set_direction(H_U0TXD, GPIO_MODE_INPUT);
        gpio_reset_pin(H_U0RXD);
        gpio_set_direction(H_U0RXD, GPIO_MODE_INPUT);
    }
    else
    {
