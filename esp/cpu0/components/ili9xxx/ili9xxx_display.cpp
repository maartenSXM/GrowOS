...
void ILI9XXXDisplay::dump_config() {
  ...
  if (this->color_order_ == display::COLOR_ORDER_BGR) {
    ESP_LOGCONFIG(TAG, "  Color order: bgr");
  } else {
    ESP_LOGCONFIG(TAG, "  Color order: rgb");
  }
  if (this->pre_invertcolors_) {
    ESP_LOGCONFIG(TAG, "  Invert colors: YES");
  }
