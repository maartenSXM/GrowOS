...
MCUFRIEND_kbv::MCUFRIEND_kbv(int CS, int RS, int WR, int RD, int _RST):Adafruit_GFX(320, 480)   //240,320 orig
{
...
//        return 0x1526;          //subsequent begin() enables Command Access
    if (ret == 0x1526) return 0x1526;          //R61526 [xx 06 15 26] if I have written NVM   //subsequent begin() enables Command Access
	if (ret == 0x89F0) return 0x7735;         //ST7735S: [xx 7C 89 F0]  
	if (ret == 0x8552) return 0x7789;         //ST7789V: [xx 85 85 52]  
    if (ret == 0xAC11) return 0xAC11;         //?unknown [xx 61 AC 11]
    ...
    msb = ret >> 8;
    if (msb == 0x93 || msb == 0x94 || msb == 0x98 || msb == 0x77 || msb == 0x16)  return ret;             //0x9488, 9486, 9340, 9341, 7796
