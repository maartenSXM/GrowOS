...
//         if (ID == 0x9327 && orientation == LANDSCAPE) orientation = 3;
    ...
    void drawPixel(int x, int y, int colour) { MCUFRIEND_kbv::drawPixel(x, y, colour);}
    void drawLine(int x1, int y1, int x2, int y2) { MCUFRIEND_kbv::drawLine(x1, y1, x2, y2, _fcolor);}
    void drawLine(int x1, int y1, int x2, int y2, unsigned int color) { MCUFRIEND_kbv::drawLine(x1, y1, x2, y2, color);}
    ...
    //Draws a box using the top left and bottom right point
    ...
    void drawRectUL(int x1, int y1, int w, int h) {
        MCUFRIEND_kbv::drawRect(x1, y1, w, h, _fcolor);
    }
    void drawRectUL(int x1, int y1, int w, int h, int color) {
        MCUFRIEND_kbv::drawRect(x1, y1, w, h, color);
    }
    ...
    void drawRoundRect(int x1, int y1, int x2, int y2, int radius, uint16_t color) {
        ...
        if (w < 0) { x1 = x2; w = -w; }
        if (h < 0) { y1 = y2; h = -h; }
        MCUFRIEND_kbv::drawRoundRect(x1, y1, w, h, radius, color);
    }
    void drawRoundRectUL(int x1, int y1, int w, int h) {
        MCUFRIEND_kbv::drawRoundRect(x1, y1, w, h, _radius, _fcolor);
    }
    void drawRoundRectUL(int x1, int y1, int w, int h, int radius) {
        MCUFRIEND_kbv::drawRoundRect(x1, y1, w, h, radius, _fcolor);
    }
    void drawRoundRectUL(int x1, int y1, int w, int h, int radius, uint16_t color) {
        MCUFRIEND_kbv::drawRoundRect(x1, y1, w, h, radius, color);
    }
    ...
    void fillRectUL(int x1, int y1, int w, int h) {
        MCUFRIEND_kbv::fillRect(x1, y1, w, h, _fcolor);
    }
    void fillRectUL(int x1, int y1, int w, int h, unsigned int color) {
        MCUFRIEND_kbv::fillRect(x1, y1, w, h, color);
    }
    ...
    void fillRoundRectUL(int x1, int y1, int w, int h) {
        MCUFRIEND_kbv::fillRoundRect(x1, y1, w, h, _radius, _fcolor);
    }
    void fillRectCenter(int x, int y, int w, int h){
        x = x - (w/2);
        y = y - (h/2); 
        MCUFRIEND_kbv::fillRect(x, y, w, h, _fcolor);
    }
    void fillRectCenter(int x, int y, int w, int h, int color){
        x = x - (w/2);
        y = y - (h/2); 
        MCUFRIEND_kbv::fillRect(x, y, w, h, color);
    }
    void drawCircle(int x, int y, int radius) { MCUFRIEND_kbv::drawCircle(x, y, radius, _fcolor);}
    void drawCircle(int x, int y, int radius, int color) { MCUFRIEND_kbv::drawCircle(x, y, radius, color);}
    void fillCircle(int x, int y, int radius) { MCUFRIEND_kbv::fillCircle(x, y, radius, _fcolor);}
    void fillCircle(int x, int y, int radius, int color) { MCUFRIEND_kbv::fillCircle(x, y, radius, color);}
    ...
    void setColor(uint16_t c1, uint16_t c2)  { MCUFRIEND_kbv::setTextColor(_fcolor = c1, _bcolor = c2); }
    ...
    void setTextSize(int sz) { MCUFRIEND_kbv::setTextSize(sz); } // ####### GFX ########
    void getTextSize(uint8_t *s) { MCUFRIEND_kbv::getTextSize(s); }
    void getTextSize(uint8_t *sx, uint8_t *sy) { MCUFRIEND_kbv::getTextSize( sx, sy); }
...
*/
   ...
   //if data is 8bit we have issues so calling Adafruit GFX via MCUFRIEND, using 8bit with pushColors and fill rect method does not work
    void drawBitmap8(int x, int y, const uint8_t *data, int sx, int sy, unsigned int color, float scale=1) {
        if(scale == 1) MCUFRIEND_kbv::drawBitmap(x, y, (const uint8_t*)data, sx, sy, color);
        //else use the adafruit_gfx added 8bit scale function with adjustment for position because of scaling ((scale/2.1) may not be correct)
        else MCUFRIEND_kbv::drawBitmap8Scale(x-sx*(scale), y-sy*(scale), (const uint8_t*)data, sx, sy, color, scale);
    }
    void drawBitmap16(int x, int y, const uint16_t *data, int sx, int sy, int scale=1) {
        uint16_t color;
    ...
    void drawBitmap(int x, int y, const uint16_t *data, int sx, int sy, unsigned int color, int scale=1) {
        MCUFRIEND_kbv::setAddrWindow(x, y, x + sx*scale - 1, y + sy*scale - 1);
        if (scale == 1) MCUFRIEND_kbv::pushColors((const uint8_t*)data, sx * sy, 1);
        else {
            for (int row = 0; row < sy; row++) {
    ...
    void drawBitmap(int x, int y, const uint16_t *data, int sx, int sy, int scale=1) {
        uint16_t color;
        MCUFRIEND_kbv::setAddrWindow(x, y, x + sx*scale - 1, y + sy*scale - 1);
        if (scale == 1) MCUFRIEND_kbv::pushColors((const uint8_t*)data, sx * sy, 1);
        else {
...
//  void dispBitmap(File inFile);
    void getTextBounds(const String &str, int16_t x, int16_t y, int16_t *x1,
        ...
        MCUFRIEND_kbv::getTextBounds(str, x, y, x1, y1, w, h);
    }
