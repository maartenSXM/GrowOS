...
void TFT_eSPI::drawBitmap8Scale(int16_t x, int16_t y, const uint8_t bitmap[],
                                    int16_t w1, int16_t h1, uint16_t color, float scale)
{
  int w2 = w1 * scale;
  int h2 = h1 * scale;
  uint8_t sBitmap[w2 * h2];    // the scaled bitmap
  uint8_t tempBitmap[w1 * h1]; // the scaled bitmap

  int16_t byteWidth = (w1 + 7) / 8;  // Bitmap scanline pad = whole byte
  int16_t byteWidth2 = (w2 + 7) / 8; // Bitmap scanline pad = whole byte
  uint8_t byte = 0;

  // move bitmap out of mem to ram
  for (int b = 0; b < h1; b++)
  {
    for (int a = 0; a < w1; a++)
    {
      if (a & 7)
        ...
        byte = pgm_read_byte(&bitmap[b * byteWidth + a / 8]);
      if (byte & 0x80)
        tempBitmap[b * w1 + a] = 1;
      else
        tempBitmap[b * w1 + a] = 0;
  ...
  for (int a = 0; a < h2; a++)
  {
    for (int b = 0; b < w2; b++)
    {
      px = floor(b * x_ratio);
      py = floor(a * y_ratio);
      sBitmap[(a * w2) + b] = tempBitmap[(int)((py * w1) + px)]; // pgm_read_byte(&bitmap[(int)((py* w1 * byteWidth) + px / 8)]);
    }
  }
  startWrite();
  for (int16_t j = 0; j < h2; j++, y++)
  {
    for (int16_t i = 0; i < w2; i++)
    {
      ...
      // fillRect(x+i*scale, y+j*scale, scale, scale+1, color);      //x,y,w,h
    }
  }
  endWrite();
}
...
/*!
   ...
   (x,y) position. For 16-bit display devices; no color reduction performed.
    ...
    @param    bitmap pointer to byte array with 16-bit color bitmap
    @param    w   Width of bitmap in pixels
    @param    h   Height of bitmap in pixels
    @param    bg   optional bg
*/
/**************************************************************************/
void TFT_eSPI::drawRGBBitmap(int16_t x, int16_t y, const uint16_t bitmap[],
                             int16_t w, int16_t h, int16_t bg)
{
  uint16_t word = 0;
  startWrite();
...
*/
/**************************************************************************/
void TFT_eSPI::charBounds(char c, int16_t *x, int16_t *y, int16_t *minx,
...
*/
/**************************************************************************/
void TFT_eSPI::getTextBounds(const char *str, int16_t x, int16_t y,
...
*/
/**************************************************************************/
void TFT_eSPI::getTextBounds(const String &str, int16_t x, int16_t y,
                             int16_t *x1, int16_t *y1, uint16_t *w,
                             uint16_t *h)
{
  if (str.length() != 0)
...
*/
/**************************************************************************/
void TFT_eSPI::getTextBounds(const __FlashStringHelper *str, int16_t x,
